...
$jscomp.polyfill = function (c, h, k, m) {
  h &&
    ($jscomp.ISOLATE_POLYFILLS
      ? $jscomp.polyfillIsolated(c, h, k, m)
      : $jscomp.polyfillUnisolated(c, h, k, m))
}
...
;(function (c) {
  ...
  var n = c.fn.dataTable,
    ...
    w = 0,
    t = function (a, b) {
      if (!n.versionCheck || !n.versionCheck('1.10.8'))
  ...
  c.extend(t.prototype, {
    ...
    _constructor: function () {
      ...
      c(b.table().body()).on('click' + d, 'th, td', function (g) {
        if (!1 !== a.s.enable) {
          var q = b.cell(this)
          q.any() && a._focus(q, null, !1, g)
        }
      })
      c(k).on('keydown' + d, function (g) {
        f || a._key(g)
      })
      if (this.c.blurable)
        c(k).on('mousedown' + d, function (g) {
          c(g.target).parents('.dataTables_filter').length && a._blur()
          c(g.target).parents().filter(b.table().container()).length ||
            c(g.target).parents('div.DTE').length ||
            c(g.target).parents('div.editor-datetime').length ||
            c(g.target).parents('div.dt-datetime').length ||
            c(g.target).parents().filter('.DTFC_Cloned').length ||
            a._blur()
        })
      if (this.c.editor) {
        var p = this.c.editor
        ...
        if (this.c.editOnFocus)
          b.on('key-focus' + d + ' key-refocus' + d, function (g, q, r, v) {
            a._editor(null, v, !0)
          })
        b.on('key' + d, function (g, q, r, v, x) {
          a._editor(r, x, !1)
        })
        c(b.table().body()).on('dblclick' + d, 'th, td', function (g) {
          !1 !== a.s.enable &&
            b.cell(this).any() &&
            ((a.s.lastFocus && this !== a.s.lastFocus.cell.node()) || a._editor(null, g, !0))
        })
      ...
      if (b.settings()[0].oFeatures.bStateSave)
        b.on('stateSaveParams' + d, function (g, q, r) {
          r.keyTable = a.s.lastFocus ? a.s.lastFocus.cell.index() : null
        })
      b.on('column-visibility' + d, function (g) {
        a._tabInput()
      })
      b.on('draw' + d, function (g) {
        a._tabInput()
        if (!a.s.focusDraw && a.s.lastFocus) {
          var q = a.s.lastFocus.relative,
            ...
            v = q.row + r.start
          0 !== r.recordsDisplay &&
            (v >= r.recordsDisplay && (v = r.recordsDisplay - 1), a._focus(v, q.column, !0, g))
        }
      })
    ...
    _blur: function (a) {
      if (this.s.enable && this.s.lastFocus) {
        ...
        a ||
          (this._updateFixedColumns(b.index().column), this._emitEvent('key-blur', [this.s.dt, b]))
      }
    },
    _clipboard: function () {
      var a = this.s.dt,
      ...
      h.getSelection &&
        (c(k).on('copy' + e, function (d) {
          ...
          !f &&
            p &&
            (d.clipboardData.setData('text/plain', p.cell.render(b.c.clipboardOrthogonal)),
            d.preventDefault())
        }),
        c(k).on('paste' + e, function (d) {
          ...
          !p ||
            ...
            h.clipboardData && h.clipboardData.getData
              ? (g = h.clipboardData.getData('Text'))
              : f.clipboardData &&
                f.clipboardData.getData &&
                (g = f.clipboardData.getData('text/plain')),
            d
              ? ((f = b._inlineOptions(p.cell.index())),
                d.inline(f.cell, f.field, f.options).set(d.displayed()[0], g).submit())
              : (p.cell.data(g), a.draw(!1)))
        }))
    },
    _columns: function () {
      var a = this.s.dt,
        ...
        if (
          ...
          var q = function () {
            ...
            p.one('open' + g, function () {
              p.off('cancelOpen' + g)
              e ||
                c('div.DTE_Field_InputControl input, div.DTE_Field_InputControl textarea').select()
              f.keys.enable(e ? 'tab-only' : 'navigation-only')
              f.on('key-blur.editor', function (v, x, y) {
                p.displayed() && y.node() === l.node() && p.submit()
              })
              e && c(f.table().container()).addClass('dtk-focus-alt')
              ...
              p.one('close' + g, function () {
                ...
                p.off(g)
                c(f.table().container()).removeClass('dtk-focus-alt')
                d.s.returnSubmit &&
                  ((d.s.returnSubmit = !1), d._emitEvent('key-return-submit', [f, l]))
              })
            })
    ...
    _focus: function (a, b, e, d) {
      ...
      if (this.s.enable) {
        ...
        if (-1 !== l.length && (a < l.start || a >= l.start + l.length))
          ...
          if (e === m || !0 === e)
            this._scroll(c(h), c(k.body), g, 'offset'),
              (e = p.table().body().parentNode),
              e !== p.table().header().parentNode &&
                ((e = c(e.parentNode)), this._scroll(e, e, g, 'position'))
          this.s.lastFocus = {
            cell: l,
            node: l.node(),
            relative: {
    ...
    _key: function (a) {
      if (this.s.waitingForDraw) a.preventDefault()
        ...
        var b = this.s.enable
        this.s.returnSubmit =
          ('navigation-only' !== b && 'tab-only' !== b) || 13 !== a.keyCode ? !1 : !0
        var e = !0 === b || 'navigation-only' === b
        if (
          b &&
          (!(0 === a.keyCode || a.ctrlKey || a.metaKey || a.altKey) || (a.ctrlKey && a.altKey))
        ) {
              ...
              if (!this.c.keys || -1 !== c.inArray(a.keyCode, this.c.keys))
                switch (a.keyCode) {
                  ...
                  case 34:
                    e &&
                      !f &&
                      (a.preventDefault(), d.page(33 === a.keyCode ? 'previous' : 'next').draw(!1))
                    break
                  case 35:
                  case 36:
                    e &&
                      ...
                      (e = this._columns()),
                      this._focus(d.cell(b[35 === a.keyCode ? b.length - 1 : e[0]]), null, !0, a))
                    break
                  case 37:
    ...
    _scroll: function (a, b, e, d) {
      ...
      f.top < g && b.scrollTop(f.top)
      f.left < q && b.scrollLeft(f.left)
      f.top + p > g + r && p < r && b.scrollTop(f.top + p - r)
      f.left + l > q + a && l < a && b.scrollLeft(f.left + l - a)
    },
    _shift: function (a, b, e) {
      ...
      if (g) {
        var q = g.cell
        q &&
          ((g = d.rows({ filter: 'applied', order: 'applied' }).indexes().indexOf(q.index().row)),
          f.serverSide && (g += f.start),
          (f = d.columns(l).indexes().indexOf(q.index().column)),
          (q = l[f]),
          'rtl' === c(d.table().node()).css('direction') &&
            ('right' === b ? (b = 'left') : 'left' === b && (b = 'right')),
          'right' === b
            ? f >= l.length - 1
              ? (g++, (q = l[0]))
              : (q = l[f + 1])
            ...
            ? g--
            : 'down' === b && g++,
          0 <= g && g < p && -1 !== c.inArray(q, l)
            ? (a && a.preventDefault(), this._focus(g, q, !0, a))
            : e && this.c.blurable
            ? this._blur()
            : a && a.preventDefault())
      }
    ...
    _tabInput: function () {
      ...
      ;-1 != e &&
        (this.s.tabInput ||
          ...
          (this.s.tabInput = e)),
        (e = this.s.dt.cell(':eq(0)', '0:visible', { page: 'current', order: 'current' }).node()) &&
  ...
  n.ext.selector.cell.push(function (a, b, e) {
    ...
    for (var f = 0, p = e.length; f < p; f++)
      ((!0 === b && a.focused(e[f])) || (!1 === b && !a.focused(e[f]))) && d.push(e[f])
    return d
  })
  c(k).on('preInit.dt.dtk', function (a, b, e) {
    'dt' === a.namespace &&
      ((a = b.oInit.keys), (e = n.defaults.keys), a || e) &&
      ((e = c.extend({}, e, a)), !1 !== a && new t(b, e))
  })
  return t
})
