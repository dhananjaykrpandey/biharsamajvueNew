...
;(function (global, factory) {
    ...
    return function (str, direction) {
      ...
      for (var i$4 = 0; i$4 < len; ++i$4) {
        ...
        if (type$3 == ',') {
          ...
          var replace =
            (i$4 && types[i$4 - 1] == '!') || (end < len && types[end] == '1') ? '1' : 'N'
  ...
  function getLineStyles(cm, line, updateFrontier) {
    if (!line.styles || line.styles[0] != cm.state.modeGen) {
      ...
      var resetState =
        line.text.length > cm.options.maxHighlightLength && copyState(cm.doc.mode, context.state)
  ...
  function findStartLine(cm, n, precise) {
    ...
    for (var search = n; search > lim; --search) {
      ...
      if (
        after &&
        (!precise ||
          search + (after instanceof SavedContext ? after.lookAhead : 0) <= doc.modeFrontier)
      ) {
        return search
      }
  ...
  function markedSpansAfter(old, endCh, isInsert) {
    ...
    if (old) {
      for (var i = 0; i < old.length; ++i) {
        ...
        if (
          endsAfter ||
          (span.from == endCh && marker.type == 'bookmark' && (!isInsert || span.marker.insertLeft))
        ) {
          var startsBefore =
    ...
    if (boxAround) {
      ...
      sticky =
        ch == 0
          ...
          : ch == lineObj.text.length
          ? 'before'
          : measureCharPrepared(cm, preparedMeasure, ch - (ltr ? 1 : 0)).bottom + widgetHeight <=
              y ==
            ltr
          ? 'after'
          : 'before'
  ...
  function drawSelectionRange(cm, range, output) {
    ...
    var rightSide =
      Math.max(display.sizerWidth, displayWidth(cm) - display.sizer.offsetLeft) - padding.right
  ...
  function visibleLines(display, doc, viewport) {
    var top =
      viewport && viewport.top != null ? Math.max(0, viewport.top) : display.scroller.scrollTop
    top = Math.floor(top - paddingTop(display))
    var bottom =
      viewport && viewport.bottom != null ? viewport.bottom : top + display.wrapper.clientHeight
  ...
  function computeSelAfterChange(doc, change) {
    ...
    for (var i = 0; i < doc.sel.ranges.length; i++) {
      var range = doc.sel.ranges[i]
      out.push(
        new Range(adjustForChange(range.anchor, change), adjustForChange(range.head, change))
      )
    }
  ...
  function setSelectionNoUndo(doc, sel, options) {
    ...
    var bias =
      (options && options.bias) || (cmp(sel.primary().head, doc.sel.primary().head) < 0 ? -1 : 1)
  ...
  function leftButtonStartDrag(cm, event, pos, behavior) {
    ...
    var mouseMove = function (e2) {
      moved =
        moved || Math.abs(event.clientX - e2.clientX) + Math.abs(event.clientY - e2.clientY) >= 10
    }
  ...
  function leftButtonSelect(cm, event, start, behavior) {
    ...
    function extendTo(pos) {
      ...
      if (behavior.unit == 'rectangle') {
        ...
        setSelection(
          doc,
          normalizeSelection(cm, startSel.ranges.slice(0, ourIndex).concat(ranges), ourIndex),
  ...
  function triggerElectric(cm, inserted) {
    ...
    for (var i = sel.ranges.length - 1; i >= 0; i--) {
      ...
      if (mode.electricChars) {
        ...
        if (
          mode.electricInput.test(getLine(cm.doc, range.head.line).text.slice(0, range.head.ch))
        ) {
          indented = indentLine(cm, range.head.line, 'smart')
        }
      }
  ...
  function addEditorMethods(CodeMirror) {
    ...
    CodeMirror.prototype = {
      ...
      heightAtLine: function (line, mode, includeWidgets) {
        ...
        return (
          intoCoordSystem(this, lineObj, { top: 0, left: 0 }, mode || 'page', includeWidgets || end)
