...
;(function (mod) {
  ...
  function searchStringForward(doc, query, start, caseFold) {
    ...
    search: for (
      var line = start.line, ch = start.ch, last = doc.lastLine() + 1 - lines.length;
      line <= last;
      line++, ch = 0
    ) {
  ...
  function searchStringBackward(doc, query, start, caseFold) {
    ...
    search: for (
      var line = start.line, ch = start.ch, first = doc.firstLine() - 1 + lines.length;
      line >= first;
      line--, ch = -1
    ) {
  ...
  function SearchCursor(doc, query, pos, options) {
    ...
    if (typeof query == 'string') {
      ...
      if (!options || options.multiline !== false)
        this.matches = function (reverse, pos) {
          return (reverse ? searchRegexpBackwardMultiline : searchRegexpForwardMultiline)(
            doc,
            query,
            pos
          )
