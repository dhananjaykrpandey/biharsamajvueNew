...
;(function (mod) {
  ...
  DiffView.prototype = {
    ...
    init: function (pane, orig, options) {
      ...
      this.orig = CodeMirror(
        pane,
        copyObj({ value: orig, readOnly: !this.mv.options.allowEditingOriginals }, copyObj(options))
      )
    ...
    if (dv.mv.options.connect == 'align') {
      ...
      if (targetPos > sInfo.top && (mix = sInfo.top / halfScreen) < 1) {
        ...
        if (botDistOther > botDist && (mix = botDist / halfScreen) < 1)
          targetPos =
            targetPos * mix + (otherInfo.height - otherInfo.clientHeight - botDist) * (1 - mix)
      }
    }
  ...
  function drawConnectorsForChunk(dv, chunk, sTopOrig, sTopEdit, w) {
    ...
    if (dv.svg) {
      ...
      var curveTop =
        ' C ' + w / 2 + ' ' + topRpx + ' ' + w / 2 + ' ' + topLpx + ' ' + (w + 2) + ' ' + topLpx
    ...
    if (dv.copyButtons) {
      var copy = dv.copyButtons.appendChild(
        elt('div', dv.type == 'left' ? '\u21dd' : '\u21dc', 'CodeMirror-merge-copy')
      )
      ...
      if (editOriginals) {
        ...
        var copyReverse = dv.copyButtons.appendChild(
          elt('div', dv.type == 'right' ? '\u21dd' : '\u21dc', 'CodeMirror-merge-copy-reverse')
        )
  ...
  function copyChunk(dv, to, from, chunk) {
    ...
    var origStart =
      chunk.origTo > from.lastLine() ? Pos(chunk.origFrom - 1) : Pos(chunk.origFrom, 0)
  ...
  function getChunks(diff) {
    ...
    for (var i = 0; i < diff.length; ++i) {
      ...
      if (tp == DIFF_EQUAL) {
        var startOff =
          !startOfLineClean(diff, i) || edit.line < startEdit || orig.line < startOrig ? 1 : 0
  ...
  function goNearbyDiff(cm, dir) {
    ...
    if (views)
      for (var i = 0; i < views.length; i++) {
        ...
        ensureDiff(dv)
        var pos =
          dir < 0 ? findPrevDiff(dv.chunks, line, isOrig) : findNextDiff(dv.chunks, line, isOrig)
