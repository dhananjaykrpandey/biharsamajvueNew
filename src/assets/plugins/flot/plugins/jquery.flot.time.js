...
;(function ($) {
  ...
  function dateTickGenerator(axis) {
    ...
    for (var i = 0; i < spec.length - 1; ++i) {
      if (
        axis.delta <
          (spec[i][0] * timeUnitSize[spec[i][1]] + spec[i + 1][0] * timeUnitSize[spec[i + 1][1]]) /
            2 &&
    ...
    if (unit === 'year') {
      ...
      if (
        ...
        opts.minTickSize !== undefined &&
        opts.minTickSize[1] === 'year'
      ) {
        size = Math.floor(opts.minTickSize[0])
      } else {
        var magn = parseFloat(
          '1e' + Math.floor(Math.log(axis.delta / timeUnitSize.year) / Math.LN10)
        )
