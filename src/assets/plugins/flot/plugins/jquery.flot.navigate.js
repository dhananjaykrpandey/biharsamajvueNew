...
;(function ($) {
  ...
  function initNevigation(plot, options) {
    ...
    function onZoomClick(e, zoomOut, amount) {
      ...
      var axes = plot
        .getXAxes()
        .concat(plot.getYAxes())
        .filter(function (axis) {
          var box = axis.box
          if (box !== undefined) {
            return (
              ec.left > box.left &&
              ec.left < box.left + box.width &&
              ec.top > box.top &&
              ec.top < box.top + box.height
            )
          }
        })
    ...
    function onMouseWheel(e, delta) {
      var maxAbsoluteDeltaOnMac = 1,
        ...
        macMagicRatio = 50,
        amount = isMacScroll
          ? 1 + Math.abs(e.originalEvent.deltaY) / macMagicRatio
          : defaultNonMacScrollAmount
    ...
    plot.pan = function (args) {
      ...
      $.each(args.axes || plot.getAxes(), function (_, axis) {
        ...
        if (d !== 0) {
          var navigationOffsetBelow = saturated.saturate(
              axis.c2p(axis.p2c(axis.min) + d) - axis.c2p(axis.p2c(axis.min))
            ),
            navigationOffsetAbove = saturated.saturate(
              axis.c2p(axis.p2c(axis.max) + d) - axis.c2p(axis.p2c(axis.max))
            )
    ...
    var shouldSnap = function (delta) {
      return (
        (Math.abs(delta.y) < SNAPPING_CONSTANT && Math.abs(delta.x) >= SNAPPING_CONSTANT) ||
    ...
    plot.smartPan = function (delta, initialState, panAxes, preventEvent, smartLock) {
      ...
      Object.keys(axes).forEach(function (axisName) {
        ...
        if (d !== 0) {
          var navigationOffsetBelow = saturated.saturate(
              axis.c2p(axis.p2c(axisMin) - (p - d)) - axis.c2p(axis.p2c(axisMin))
            ),
            navigationOffsetAbove = saturated.saturate(
              axis.c2p(axis.p2c(axisMax) - (p - d)) - axis.c2p(axis.p2c(axisMax))
            )
          ...
          axis.options.offset.below = saturated.saturate(
            navigationOffsetBelow + (axis.options.offset.below || 0)
          )
          axis.options.offset.above = saturated.saturate(
            navigationOffsetAbove + (axis.options.offset.above || 0)
          )
        }
      })
    ...
    function drawOverlay(plot, ctx) {
      if (panHint) {
        ...
        if (panHint.end === false) {
          ...
          ctx.moveTo(
            startx - (dirX ? 0 : PANHINT_LENGTH_CONSTANT),
            starty - (dirX ? PANHINT_LENGTH_CONSTANT : 0)
          )
          ctx.lineTo(
            startx + (dirX ? 0 : PANHINT_LENGTH_CONSTANT),
            starty + (dirX ? PANHINT_LENGTH_CONSTANT : 0)
          ...
          ctx.moveTo(
            endx - (dirX ? 0 : PANHINT_LENGTH_CONSTANT),
            endy - (dirX ? PANHINT_LENGTH_CONSTANT : 0)
          )
          ctx.lineTo(
            endx + (dirX ? 0 : PANHINT_LENGTH_CONSTANT),
            endy + (dirX ? PANHINT_LENGTH_CONSTANT : 0)
