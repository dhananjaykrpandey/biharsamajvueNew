...
;(function (factory) {
  ...
  Mapael.prototype = {
    ...
    init: function () {
      ...
      if (
        self.options.map.cssClass === '' ||
        $('.' + self.options.map.cssClass, self.container).length === 0
      ) {
      ...
      if ($[pluginName] && $[pluginName].maps && $[pluginName].maps[self.options.map.name]) {
        ...
        $.fn[pluginName] &&
        $.fn[pluginName].maps &&
        $.fn[pluginName].maps[self.options.map.name]
      ) {
        // Mapael version <= 1.x - DEPRECATED
        self.mapConf = $.fn[pluginName].maps[self.options.map.name]
        if (window.console && window.console.warn) {
          window.console.warn(
            "Extending $.fn.mapael is deprecated (map '" + self.options.map.name + "')"
          )
        }
      } else {
      ...
      if (self.options.map.width) {
        ...
        self.paper.setSize(
          self.options.map.width,
          self.mapConf.height * (self.options.map.width / self.mapConf.width)
        )
      } else {
      ...
      $(self.paper.desc).append(
        ' and Mapael ' + self.version + ' (https://www.vincentbroute.fr/mapael/)'
      )
    },
      ...
      $.each(['area', 'plot'], function (key, type) {
        if ($.isArray(options.legend[type])) {
          for (var i = 0; i < options.legend[type].length; ++i)
            options.legend[type][i] = $.extend(
              true,
              {},
              Mapael.prototype.legendDefaultOptions[type],
              options.legend[type][i]
            )
        } else {
          options.legend[type] = $.extend(
            true,
            {},
            Mapael.prototype.legendDefaultOptions[type],
            options.legend[type]
      ...
      self.$container.on('click.' + pluginName, '[data-id]', function (evt, opts) {
        ...
        if (dataTypeToElementMapping[type] !== undefined) {
          ...
          self.handleClickOnLegendElem(
            self.legends[legendType][legendIndex].elems[id],
            id,
            legendIndex,
            legendType,
            opts
    ...
    initElem: function (id, type, elem) {
      ...
      if (elem.options.text && elem.options.text.content !== undefined) {
        ...
        var textPosition = self.getTextPosition(
          elem.mapElem.getBBox(),
          elem.options.text.position,
          elem.options.text.margin
        )
    ...
    initZoom: function (mapWidth, mapHeight, zoomOptions) {
      ...
      if (self.options.map.zoom.touch) {
        self.$map.on('touchstart.' + pluginName, function (e) {
          if (e.originalEvent.touches.length === 2) {
            self.zoomCenterX =
              (e.originalEvent.touches[0].pageX + e.originalEvent.touches[1].pageX) / 2
            self.zoomCenterY =
              (e.originalEvent.touches[0].pageY + e.originalEvent.touches[1].pageY) / 2
            self.previousPinchDist = Math.sqrt(
        ...
        self.$map.on('touchmove.' + pluginName, function (e) {
          ...
          if (e.originalEvent.touches.length === 2) {
            ...
            if (Math.abs(pinchDist - self.previousPinchDist) > 15) {
              ...
              zoomLevel =
                (pinchDist - self.previousPinchDist) / Math.abs(pinchDist - self.previousPinchDist)
              self.$container.trigger('zoom', {
                fixedCenter: true,
                level: self.zoomData.zoomLevel + zoomLevel,
                x: coord.x,
      ...
      $('body').on(
        'mouseup.' + pluginName + (zoomOptions.touch ? ' touchend.' + pluginName : ''),
        function () {
          mousedown = false
      ...
      self.$map
        .on(
          'mousedown.' + pluginName + (zoomOptions.touch ? ' touchstart.' + pluginName : ''),
          function (e) {
            clearTimeout(panningMouseUpTO)
            clearTimeout(panningMouseMoveTO)
        ...
        .on(
          'mousemove.' + pluginName + (zoomOptions.touch ? ' touchmove.' + pluginName : ''),
          function (e) {
            ...
            if (mousedown && currentLevel !== 0) {
              var offsetX =
                ((previousX - pageX) / (1 + currentLevel * zoomOptions.step)) *
                (mapWidth / self.paper.width)
              var offsetY =
                ((previousY - pageY) / (1 + currentLevel * zoomOptions.step)) *
                (mapHeight / self.paper.height)
              var panX = Math.min(
                Math.max(0, self.currentViewBox.x + offsetX),
                mapWidth - self.currentViewBox.w
              )
              var panY = Math.min(
                Math.max(0, self.currentViewBox.y + offsetY),
                mapHeight - self.currentViewBox.h
              )
    ...
    mapPagePositionToXY: function (pageX, pageY) {
      ...
      var initFactor = self.options.map.width
        ? self.mapConf.width / self.options.map.width
        : self.mapConf.width / self.$map.width()
    ...
    onZoomEvent: function (e, zoomOptions) {
      ...
      var animDuration =
        zoomOptions.animDuration !== undefined
          ? zoomOptions.animDuration
          : self.options.map.zoom.animDuration
      ...
      zoomLevel = Math.min(
        Math.max(zoomLevel, self.options.map.zoom.minLevel),
        self.options.map.zoom.maxLevel
      )
        ...
        if (zoomOptions.fixedCenter !== undefined && zoomOptions.fixedCenter === true) {
          panX =
            ...
            ((zoomOptions.x - self.zoomData.panX) *
              (relativeZoomLevel - previousRelativeZoomLevel)) /
              relativeZoomLevel
          panY =
            self.zoomData.panY +
            ((zoomOptions.y - self.zoomData.panY) *
              (relativeZoomLevel - previousRelativeZoomLevel)) /
              relativeZoomLevel
        } else {
      ...
      if (
        relativeZoomLevel === previousRelativeZoomLevel &&
        panX === self.zoomData.panX &&
        panY === self.zoomData.panY
      )
        return
      ...
      if (animDuration > 0) {
        self.animateViewBox(
          ...
          panHeight,
          animDuration,
          self.options.map.zoom.animEasing
        )
      } else {
    ...
    showElemByRange: function (ranges, elems, hiddenOpacity, animDuration) {
      ...
      $.each(ranges, function (valueIndex) {
        ...
        $.each(elems, function (id) {
          ...
          // Check if in range
          if (
            (range.min !== undefined && elemValue[valueIndex] < range.min) ||
    ...
    onUpdateEvent: function (e, opt) {
      ...
      if (typeof opt.mapOptions === 'object') {
        ...
        if (
          opt.mapOptions.areas !== undefined ||
          opt.mapOptions.plots !== undefined ||
          opt.mapOptions.legend !== undefined
        ) {
      ...
      $.each(self.areas, function (id) {
        ...
        if (
          (typeof opt.mapOptions === 'object' &&
            ((typeof opt.mapOptions.map === 'object' &&
              typeof opt.mapOptions.map.defaultArea === 'object') ||
              (typeof opt.mapOptions.areas === 'object' &&
                typeof opt.mapOptions.areas[id] === 'object') ||
              (typeof opt.mapOptions.legend === 'object' &&
                typeof opt.mapOptions.legend.area === 'object'))) ||
      ...
      $.each(self.plots, function (id) {
        ...
        if (
          (typeof opt.mapOptions === 'object' &&
            ((typeof opt.mapOptions.map === 'object' &&
              typeof opt.mapOptions.map.defaultPlot === 'object') ||
              (typeof opt.mapOptions.plots === 'object' &&
                typeof opt.mapOptions.plots[id] === 'object') ||
              (typeof opt.mapOptions.legend === 'object' &&
                typeof opt.mapOptions.legend.plot === 'object'))) ||
          opt.replaceOptions === true
        ) {
      ...
      $.each(self.links, function (id) {
        ...
        if (
          (typeof opt.mapOptions === 'object' &&
            ((typeof opt.mapOptions.map === 'object' &&
              typeof opt.mapOptions.map.defaultLink === 'object') ||
              (typeof opt.mapOptions.links === 'object' &&
                typeof opt.mapOptions.links[id] === 'object'))) ||
          opt.replaceOptions === true
        ) {
        ...
        $.each(opt.setLegendElemsState, function (legendCSSClass, action) {
          ...
          if ($legend !== undefined) {
            ...
            $("[data-type='legend-elem']", $legend).each(function (id, elem) {
              if (
                ($(elem).attr('data-hidden') === '0' && action === 'hide') ||
    ...
    setPlotCoords: function (plot) {
      ...
      if (plot.options.x !== undefined && plot.options.y !== undefined) {
        ...
        plot.options.plotsOn !== undefined &&
    ...
    setPlotAttributes: function (plot) {
      if (plot.options.type === 'square') {
        ...
        plot.mapElem.baseTransform =
          'm' +
          plot.options.width / plot.mapElem.originalBBox.width +
          ',0,0,' +
    ...
    drawLink: function (id, xa, ya, xb, yb, elemOptions) {
      ...
      var c =
        ...
        yc * bcd +
        yc * yc -
        elemOptions.factor * abDist * (elemOptions.factor * abDist)
      var delta = b * b - 4 * a * c
      var x = 0
      var y = 0
      ...
      link.mapElem = self.paper.path(
        'm ' + xa + ',' + ya + ' C ' + x + ',' + y + ' ' + xb + ',' + yb + ' ' + xb + ',' + yb + ''
      )
    ...
    isAttrsChanged: function (originalAttrs, newAttrs) {
      for (var key in newAttrs) {
        if (
          (newAttrs.hasOwnProperty(key) && typeof originalAttrs[key] === 'undefined') ||
    ...
    drawLegend: function (legendOptions, legendType, elems, scale, legendIndex) {
      ...
      if (legendOptions.title && legendOptions.title !== '') {
        title = legendPaper
          .text(legendOptions.marginLeftTitle, 0, legendOptions.title)
      ...
      for (i = 0; i < legendOptions.slices.length; ++i) {
        ...
        sliceOptions[i] = $.extend(
          true,
          {},
          legendType === 'plot' ? self.options.map.defaultPlot : self.options.map.defaultArea,
          legendOptions.slices[i]
        )
        ...
        if (
          legendType === 'plot' &&
          (sliceOptions[i].type === undefined || sliceOptions[i].type === 'circle')
        ) {
      ...
      for (i = 0; i < sliceOptions.length; ++i) {
        ...
        if (sliceOptions[i].display === undefined || sliceOptions[i].display === true) {
          if (legendType === 'area') {
            ...
            legendElem = legendPaper.rect(
              x,
              y,
              scale * sliceOptions[i].attrs.width,
              scale * sliceOptions[i].attrs.height
            )
          } else if (sliceOptions[i].type === 'square') {
            if (legendOptions.mode === 'horizontal') {
              x = width + legendOptions.marginLeft
            ...
            if (sliceOptions[i].type === 'image') {
              ...
              sliceOptions[i].attrs.transform =
                'm' +
                (scale * sliceOptions[i].width) / legendElemBBox.width +
                ',0,0,' +
                (scale * sliceOptions[i].height) / legendElemBBox.height +
                ',' +
                x +
                ',' +
                y +
          ...
          if (legendOptions.hideElemsOnClick.enabled) {
            ...
            self.setHoverOptions(
              legendLabel,
              legendOptions.labelAttrs,
              legendOptions.labelAttrsHover
            )
    ...
    handleClickOnLegendElem: function (elem, id, legendIndex, legendType, opts) {
      ...
      var animDuration =
        opts.animDuration !== undefined
          ? opts.animDuration
          : legendOptions.hideElemsOnClick.animDuration
      ...
      $.each(mapElems, function (y) {
        ...
        if (self.getLegendSlice(elemValue, legendOptions) === sliceOptions) {
          if (hidden === '0') {
            ...
            self.setElementOpacity(
              mapElems[y],
              legendOptions.hideElemsOnClick.opacity,
              animDuration
            )
          } else {
            ...
            if ($.isEmptyObject(hiddenBy)) {
              self.setElementOpacity(
                mapElems[y],
                mapElems[y].mapElem.originalAttrs.opacity !== undefined
                  ? mapElems[y].mapElem.originalAttrs.opacity
                  : 1,
                animDuration
              )
            }
      ...
      if (
        (opts.hideOtherElems === undefined || opts.hideOtherElems === true) &&
        legendOptions.exclusive === true
      ) {
    ...
    createLegends: function (legendType, elems, scale) {
      ...
      for (var j = 0; j < legendsOptions.length; ++j) {
        if (
          legendsOptions[j].display === true &&
          $.isArray(legendsOptions[j].slices) &&
          legendsOptions[j].slices.length > 0 &&
          legendsOptions[j].cssClass !== '' &&
          $('.' + legendsOptions[j].cssClass, self.$container).length !== 0
        ) {
          self.legends[legendType][j] = self.drawLegend(
            legendsOptions[j],
            legendType,
            elems,
            scale,
    ...
    elemOut: function (elem) {
      ...
      if (elem.textElem !== undefined) {
        self.animate(
          elem.textElem,
          elem.textElem.originalAttrs,
          elem.textElem.attrsHover.animDuration
        )
      }
        ...
        if ($.isArray(legendOptions)) {
          for (var i = 0; i < legendOptions.length; ++i) {
            options = $.extend(
              ...
              {},
              options,
              self.getLegendSlice(options.value[i], legendOptions[i])
            )
          }
        } else {
    ...
    getLegendSlice: function (value, legend) {
      for (var i = 0; i < legend.slices.length; ++i) {
        if (
          (legend.slices[i].sliceValue !== undefined && value === legend.slices[i].sliceValue) ||
    ...
    animateViewBox: function (targetX, targetY, targetW, targetH, duration, easingFunction) {
      ...
      var computeNextStep = function () {
        ...
        if (elapsed < durationWithMargin) {
          ...
          if (
            oldZoomAnimStartTime &&
            self.zoomAnimCVBTarget &&
            self.zoomAnimCVBTarget.w !== targetW
          ) {
