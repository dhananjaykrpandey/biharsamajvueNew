...
;(function (n) {
  ...
  function O(a, c) {
    J === B &&
      (J =
        -1 ===
        M.serializeToString(
          new u.DOMParser().parseFromString(P['xl/worksheets/sheet1.xml'], 'text/xml')
        ).indexOf('xmlns:r'))
  ...
  D.ext.buttons.excelHtml5 = {
    ...
    action: function (a, c, d, b) {
      ...
      var p = c.buttons.exportData(b.exportOptions),
        ...
        q = function (k) {
          ...
          for (var l = 0, z = k.length; l < z; l++) {
            ...
            A ||
              ('number' === typeof k[l] ||
              (k[l].match && k[l].match(/^-?\d+(\.\d+)?([eE]\-?\d+)?$/) && !k[l].match(/^0\d+/))
                ...
                : ((H = H.replace ? H.replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g, '') : H),
                  (A = y(f, 'c', {
                    ...
                    children: {
                      row: y(f, 'is', {
                        children: { row: y(f, 't', { text: H, attr: { 'xml:space': 'preserve' } }) }
                      })
                    }
                  }))))
            h.appendChild(A)
      ...
      for (C = p.header.length; v < C; v++)
        q.appendChild(
          y(f, 'col', { attr: { min: v + 1, max: v + 1, width: V(p, v), customWidth: 1 } })
        )
      q = a.xl['workbook.xml']
      n('sheets sheet', q).attr('name', Q(b))
      b.autoFilter &&
        (n('mergeCells', f).before(
          y(f, 'autoFilter', { attr: { ref: 'A' + d + ':' + I(p.header.length - 1) + w } })
        ),
  ...
  D.ext.buttons.pdfHtml5 = {
    ...
    action: function (a, c, d, b) {
      ...
      a.messageTop &&
        d.content.unshift({ text: a.messageTop, style: 'message', margin: [0, 0, 0, 12] })
      a.messageBottom &&
        d.content.push({ text: a.messageBottom, style: 'message', margin: [0, 0, 0, 12] })
